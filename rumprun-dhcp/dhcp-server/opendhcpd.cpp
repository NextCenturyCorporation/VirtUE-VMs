...
MYWORD gdmess(data9 *req, MYBYTE sockInd)
{
    ...
    if (sockInd == 255)
    {
        ...
        //printf("RADDR = %s\n", inet_ntoa(req->msgcontrol.   pktinfo.ipi_spec_dst));
	 	...
	 	#ifdef RUMPRUN
			// Netbsd does not support ipi_spec_dst so just take the address of the first interface
			// This change means that netbsd would only support one interface
			MYDWORD addr = network.dhcpConn[0].server;
		#else
